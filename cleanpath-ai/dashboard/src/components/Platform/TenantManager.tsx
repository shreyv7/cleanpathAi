    ...
    return (
        <div className="glass rounded-2xl p-6 border border-white/5 bg-[#080B10]/80 relative overflow-hidden flex flex-col gap-4 h-[420px]">
            ...
            <div>
                ...
                <p className="text-[10px] text-white/40">Examine role segmentations, agency hierarchies, and regional control logs</p>
            </div>
            ...
            <div className="flex-1 overflow-y-auto investigation-scroll pr-1 flex flex-col gap-3 font-mono text-xs">
                {BUSINESS_TENANTS.map((t) => (
                    <div
                        key={t.id}
                        className="p-3.5 rounded-xl border border-white/5 bg-[#03060a]/60 hover:bg-[#05080f]/80 hover:border-white/10 transition-all duration-300 flex flex-col gap-2"
                    >
                        <div className="flex justify-between items-start">
                            <div className="space-y-0.5">
                                <span className="text-xs font-bold text-white/95">{t.organization}</span>
                                <div className="text-[9px] text-white/30">BU TYPE: {t.unitType}</div>
                            </div>
                            <span className="text-[8.5px] bg-purple-500/10 border border-purple-500/20 text-purple-400 font-bold px-1.5 py-0.5 rounded">
                                {t.region}
                            </span>
                        </div>

                        <div className="flex justify-between items-center text-[9px] text-white/30 border-t border-white/5 pt-2 mt-0.5">
                            <span>Audited Spend: <span className="text-white/70 font-bold">${(t.spendAudited / 1000000).toFixed(1)}M</span></span>
                            <span>•</span>
                            <span>Inherited Policies: <span className="text-white/70 font-bold">{t.inheritedPolicyCount} Rules</span></span>
                        </div>
                    </div>
                ))}
            </div>
