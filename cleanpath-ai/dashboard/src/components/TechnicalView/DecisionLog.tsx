    ...
    return (
        <div className="flex flex-col gap-6 w-full">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <h2 className="text-2xl font-bold text-white">Technical Decision Log</h2>
                    <p className="text-white/40 text-sm mt-1">Real-time edge classification audit trail</p>
                </div>
            ...
            <div className="glass rounded-2xl border border-white/5 bg-white/5 overflow-hidden">
                <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="border-b border-white/5 bg-white/[0.02]">
                                <th className="px-6 py-4 text-xs font-semibold text-white/40 uppercase tracking-wider"><div className="flex items-center gap-2"><Clock className="w-3 h-3" /> Timestamp</div></th>
                                <th className="px-6 py-4 text-xs font-semibold text-white/40 uppercase tracking-wider"><div className="flex items-center gap-2"><Globe className="w-3 h-3" /> Publisher ID</div></th>
                                <th className="px-6 py-4 text-xs font-semibold text-white/40 uppercase tracking-wider"><div className="flex items-center gap-2"><Shield className="w-3 h-3" /> Score</div></th>
                                <th className="px-6 py-4 text-xs font-semibold text-white/40 uppercase tracking-wider">Decision</th>
                                <th className="px-6 py-4 text-xs font-semibold text-white/40 uppercase tracking-wider text-right"><div className="flex items-center gap-2 justify-end"><Hash className="w-3 h-3" /> Latency</div></th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-white/5">
                            {loading ? (
                                ...
                                <tr>
                                    <td colSpan={5} className="px-6 py-12 text-center text-white/20 text-sm">No decisions found for selected filters</td>
                                </tr>
                            ) : (
                                data?.data.map((entry) => (
                                    <tr key={entry.id} className="hover:bg-white/[0.02] transition-colors group cursor-pointer" onClick={() => router.push(`/investigate/${encodeURIComponent(entry.publisher_id)}`)}>
                                        ...
                                        <td className="px-6 py-4 text-sm text-white/90 font-medium whitespace-nowrap">
                                            {entry.publisher_id}
                                        </td>
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className="text-xs text-white/50">{entry.thermal_score}</span>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4">
                                            {getDecisionBadge(entry.decision)}
                                        </td>
                                        <td className="px-6 py-4 text-sm text-right font-mono text-blue-400 font-bold">
                                            {entry.latency_ms}ms
                                        </td>
                                    </tr>
                                ))
