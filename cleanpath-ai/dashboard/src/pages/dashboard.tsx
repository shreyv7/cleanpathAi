...
export default function Home() {
    ...
    return (
        <main className="flex min-h-screen flex-col items-center p-12 bg-background text-foreground transition-colors duration-300">
            {signupData && (
                <div className="w-full max-w-7xl mb-6 bg-blue-500/5 dark:bg-blue-500/5 border border-blue-500/10 dark:border-blue-500/20 rounded-2xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 animate-in slide-in-from-top-4 duration-300">
                    <div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-xl bg-blue-500/10 border border-blue-500/20 flex items-center justify-center">
                            <Database className="w-4 h-4 text-blue-500 dark:text-blue-400" />
                        </div>
                        <div>
                            <div className="text-[10px] font-mono text-blue-500 dark:text-blue-400 uppercase tracking-widest font-bold">Active Programmatic Sandbox</div>
                            <div className="text-sm font-semibold text-foreground/90 dark:text-white/90">
                                Tenant: <span className="text-foreground dark:text-white">{signupData.company}</span> • Connected DSP: <span className="text-foreground dark:text-white uppercase font-mono">{signupData.dsp}</span>
                            </div>
                        </div>
                    </div>
                    <div className="flex items-center gap-2">
                        <span className="text-[10px] bg-blue-500/10 border border-blue-500/20 text-blue-500 dark:text-blue-400 font-mono font-bold px-2 py-0.5 rounded-full uppercase">
                        ...
                        <button
                            ...
                            className="text-xs text-foreground/40 dark:text-white/40 hover:text-foreground/80 dark:hover:text-white/80 transition-colors"
                        >
                            Reset
                        </button>
                    </div>
            ...
            <div className="z-10 max-w-7xl w-full flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-16 border-b border-border pb-6">
                {/* Brand Logo & Telemetry Switcher */}
                <div className="flex flex-wrap items-center gap-4">
                    <div className="flex items-center gap-2">
                        <ShieldCheck className="w-5 h-5 text-foreground/80 dark:text-white/80" />
                        <span className="text-sm font-bold tracking-wider text-foreground dark:text-white">
                            CLEANPATH
                        </span>
                        <span className="text-[10px] text-foreground/40 dark:text-white/40 bg-muted dark:bg-white/5 border border-border dark:border-white/10 font-mono px-1.5 py-0.5 rounded">
                            L4-SECURE
                        </span>
                    </div>
                ...
                <div className="flex flex-wrap items-center gap-2 text-xs">
                    <button
                        ...
                        className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-card border border-border text-foreground/60 hover:text-foreground hover:border-foreground/10 hover:bg-muted dark:bg-[#0A0C10] dark:border-white/5 dark:text-white/60 dark:hover:text-white dark:hover:border-white/10 dark:hover:bg-white/[0.01] transition-all cursor-pointer font-medium"
                    >
            ...
            <div className="max-w-7xl w-full flex flex-col gap-24">
                ...
                <div id="executive-summary" className="scroll-mt-12 flex flex-col gap-2">
                    <h1 className="text-5xl font-extrabold tracking-tighter text-foreground dark:text-white">
                        Executive Summary
                    </h1>
                    <p className="text-foreground/50 dark:text-white/50 text-xl">
